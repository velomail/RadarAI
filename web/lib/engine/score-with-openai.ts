...
const SYSTEM_PROMPT_BASE = [
  ...
  '- job_title must match the JOB Title field exactly (do not rewrite or summarize the title).',
  ...
  '',
  'Narrative rules (critical for the user — write like a polished Indeed/LinkedIn listing plus a recruiter note):',
  '- why_promising: 3-5 sentences addressed directly to the candidate. Open with why THIS role is worth their attention given THEIR resume. Name 2-3 concrete resume facts (titles, tools, industries, metrics) and tie each to a requirement from the posting. Close with one realistic upside or honest caveat.',
  '- role_summary: 2-4 sentences describing the role the way Indeed or LinkedIn would — lead with what the person will do day-to-day, then company context, employment type, and location/remote pattern. Skimmable plain prose, no bullets, no "Job Description" header text.',
  '- experience_match: 3-5 sentences as a personalized fit analysis. Compare resume evidence to posting requirements sentence by sentence. Quote or paraphrase specific resume lines and map them to duties or qualifications in the description. Mention gaps only when material.',
  '- key_advantages: 3-5 short bullet lines separated by newline (start each with "• "). Each line maps one resume strength to one posting requirement (max ~100 chars per line).',
  '- gaps_or_objections: 0-3 short bullet lines separated by newline (start each with "• "). Only material gaps — missing credential, schedule conflict, location mismatch, thin industry overlap.',
  '',
